...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, indexData: int):
        ...
        imagedata = read_pkl_data(file_path, NO_VALUE)
        return imagedata
    ...
    def collate_fn(self, batch: list):
        ...
        """
        return torch.concat([oneTensor.unsqueeze(0) for oneTensor in batch], dim=0)


def read_pkl_data(file_path: str, no_value: float):
    try:
        with open(file_path, 'rb') as file:
            data = pickle.load(file)
    except:
        raise FileNotFoundError(f"The data file {file_path} does not exist.")
    # data['normals'][numpy.isnan(data['normals'])] = no_value
