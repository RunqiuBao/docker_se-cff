...
class CocoSegmentation(coco.CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        for category in target:
            ...
            if len(seg_rle) > 0:
                category['segmentation'] = decode(frPyObjects(seg_rle, self._imageHeight, self._imageWidth))
        return img, target


def DetectCornersFromMask(mask: numpy.ndarray):
    contours = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
    if contours is None or len(contours) == 0:
        print("contours: {}, mask: {}".format(contours, mask.sum()))
        cv2.imwrite("/root/code/debug_mask.png", mask * 255)
    contour = max(contours, key=cv2.contourArea)
    for factor in numpy.linspace(0.001, 0.1, 100):
        approx = cv2.approxPolyDP(contour, factor * cv2.arcLength(contour, True), True)
        if len(approx) == 4:
            break
    # epsilon = 0.008 * cv2.arcLength(contour, True)
    # approx = cv2.approxPolyDP(contour, epsilon, True)
    assert approx is not None and approx.shape[0] == 4, "approx: {}".format(approx)
        ...
        if self._isLoadCOCOFormat:
            ...
            try:
                for indexTarget in range(len(left_targets)):
                    ...
                    cocoSegMap_right = right_targets[indexTarget]["segmentation"]
                    if isinstance(cocoSegMap, numpy.ndarray):
                        leftmask = cocoSegMap[:, :self._imageSize[0]].squeeze()
                        left_corners = DetectCornersFromMask(leftmask)  # (4, 2) shape
                        rightmask = cocoSegMap_right[:, self._imageSize[0]:].squeeze()
                        right_corners = DetectCornersFromMask(rightmask)
                    else:
                        numpy.zeros((self._imageSize[1], self._imageSize[0]))
                        left_corners = None
