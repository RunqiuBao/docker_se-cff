...
class EventDataset(torch.utils.data.Dataset):
    _PATH_DICT = {"timestamp": "timestamps.txt", "left": "left", "right": "right"}
    ...
    def __getitem__(self, x):
        ...
        print("event_data shape:", event_data['left'].shape)
        return event_data
    ...
    def _pre_load_event_data(self, timestamp):
        ...
        for location in self._LOCATION:
            mask = np.logical_and(
                minimum_time <= event_data[location]["t"],
                event_data[location]["t"] <= maximum_time,
            )
            for data_type in ["x", "y", "t", "p"]:
                event_data[location][data_type] = event_data[location][data_type][mask]
            event_data['raw'][location] = event_data[location]

        for location in self._LOCATION:
            event_data[location] = self.stack_function.pre_stack(
    ...
    def collate_fn(batch):
        rawEvents = []
        for indexEntry, oneEntry in enumerate(batch):
            if 'raw' in oneEntry:
                rawEvents.append(batch[indexEntry]['raw'])
                del batch[indexEntry]['raw']
        batch = torch.utils.data._utils.collate.default_collate(batch)
        if len(rawEvents) > 0:
            batch['raw'] = {}
            for indexEntry, oneEntry in enumerate(rawEvents):
                batch['raw'][indexEntry] = oneEntry
