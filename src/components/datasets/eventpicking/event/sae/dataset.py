...
class EventDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, x):
        ...
        if self.lmdb_txn is not None:
            ...
            event_data = self._stack_func(timestamp=timestamp)
            for key, value in event_data.items():
                event_data[key] = value.transpose(2, 0, 1)
        return event_data

    def _stack_func(self, timestamp):
        event_data = {}
        ...
        for location in self._LOCATION:
            event_data[location] = self.stack_function.do_stack(event_data[location])
