...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.event_dataset = event_module.EventDataset(
            root=event_root, **event_cfg.PARAMS
        )
        ...
        self.disparity_dataset = disparity_module.DisparityDataset(
            root=disparity_root, **disparity_cfg.PARAMS
        )
        ...
        if split in ["train", "valid", "trainval"]:
            ...
            if event_cfg.NAME != "none":
                minimum_timestamp = max(
                    self.event_dataset.event_slicer["left"].t_offset,
                ...
                if event_cfg.NAME == "base":
                    ...
                    minimum_timestamp = max(
                        self.event_dataset.event_slicer["left"].min_time,
                        self.event_dataset.event_slicer["right"].min_time,
                    )
                    maximum_timestamp = min(
                        self.event_dataset.event_slicer["left"].max_time,
                ...
                if maximum_timestamp is not None:
                    self.timestamps = self.timestamps[
                        self.timestamps <= maximum_timestamp
                    ]
            self.timestamp_to_index = copy.copy(
                self.disparity_dataset.timestamp_to_index
            )
        elif split in ["test"]:
            self.timestamps, self.timestamp_to_index = read_csv(
                os.path.join(root, self.sequence_name + ".csv")
            )
        else:
            raise NotImplementedError

        self.timestamps = self.timestamps[
            [idx for idx in range(0, len(self.timestamps), sampling_ratio)]
        ]
    ...
    def collate_fn(self, batch):
        ...
        domain = "disparity"
        if domain in batch[0].keys():
            output[domain] = self.disparity_dataset.collate_fn(
                [sample[domain] for sample in batch]
            )
