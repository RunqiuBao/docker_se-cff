...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.disparity_dataset = disparity_module.DisparityDataset(img_metadata=img_metadata)
        ...
        if split in ['train', 'trainval']:
            ...
            if kwargs.get('randomhorizontalflip', False):
                transformsList.append(transforms.RandomHorizontalFlip(
            ...
            transformsList.append(transforms.Padding(event_module=event_module,
                    img_height=crop_height,
                    img_width=crop_width,
                    no_event_value=self.event_dataset.NO_VALUE,
                    no_disparity_value=self.disparity_dataset.NO_VALUE,
                    disparity_module=disparity_module))
            transformsList.append(transforms.ToTensor(event_module=event_module, disparity_module=disparity_module, objdet_module=objdet_module))
            ...
            self.transforms = transforms.Compose([
                transforms.Padding(event_module=event_module,
                                   ...
                                   no_disparity_value=self.disparity_dataset.NO_VALUE,
                                   disparity_module=disparity_module),
                transforms.ToTensor(event_module=event_module, disparity_module=disparity_module, objdet_module=objdet_module),
            ])
        else:
            raise NotImplementedError
    ...
    def collate_fn(self, batch):
        ...
        domain = 'objdet'
        if domain in batch[0].keys():
            output[domain] = self.objdet_dataset.collate_fn([oneInstance[domain] for oneInstance in batch])
        ...
        output['event']['left'] = output['event']['left'].permute(0, 2, 3, 1).unsqueeze(1).unsqueeze(4).to(torch.float32)
        output['event']['right'] = output['event']['right'].permute(0, 2, 3, 1).unsqueeze(1).unsqueeze(4).to(torch.float32)
        return output
