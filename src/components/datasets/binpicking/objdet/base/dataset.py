    ...
    def FormatLabels(self, labels_data, indexFrame, imageData, imageId):
        ...
        for oneGroup in groupedAnnotations.values():
            if len(oneGroup) > 2: # containing separate points.
                left_target_group, right_target_group = {"points": [], "bbox": None}, {"points": [], "bbox": None}
                for oneTargetInOneGroup in oneGroup:
                    if isinstance(oneTargetInOneGroup, Points):
                        if oneTargetInOneGroup.points[0] < self._imageSize[0]:
                            left_target_group["points"].append(oneTargetInOneGroup)
                        else:
                            right_target_group["points"].append(oneTargetInOneGroup)
                ...
                if left_target_group != {"points": [], "bbox": None}:
                    left_targets.append(left_target_group)
                if right_target_group != {"points": [], "bbox": None}:
        ...
        try:
            for indexTarget in range(len(left_targets)):
                ...
                if isinstance(left_target, Polygon):
                    ...
                    left_corners = numpy.array([onePoint.points for onePoint in left_target_points]).reshape(-1, 2)
                    leftcorners.append(
                        FormatKeypoints(left_corners, self._max_num_keypoints)[None, :]
                    )
                    right_corners = numpy.array([onePoint.points for onePoint in right_target_points]).reshape(-1, 2)
                    right_corners[:, 0] -= self._imageSize[0]
                    rightcorners.append(
                        FormatKeypoints(right_corners, self._max_num_keypoints)[None, :]
                    )
                else:
