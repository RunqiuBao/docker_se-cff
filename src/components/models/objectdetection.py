...
class Cylinder5DDetectionHead(nn.Module):
        ...
        for _ in self._config["strides"]:
            self._multi_level_cls_convs.append(
                self._build_objdet_convs(
                    ...
                    * 2,  # Note: concat left and right feature maps
            ...
            self._multi_level_keypts1_convs.append(
                self._build_keypts_convs(
                    in_channels=self._config["feat_channels"],
                    feat_channels=self._config["feat_channels"],
                    num_classes=self._config["num_classes"],
                )
            )
            self._multi_level_keypts2_convs.append(
                self._build_keypts_convs(
                    in_channels=self._config["feat_channels"],
                    feat_channels=self._config["feat_channels"],
                    num_classes=self._config["num_classes"],
    ...
    def __init_weights(self):
        ...
        for module_list in [
            ...
            self._multi_level_keypts1_convs,
            self._multi_level_keypts2_convs,
        ]:
    ...
    def _build_keypts_convs(in_channels: int, feat_channels: int, num_classes: int):
        ...
        stacked_convs.append(
            nn.ConvTranspose2d(
                feat_channels, feat_channels, kernel_size=4, stride=2, padding=1
            )
        )
        stacked_convs.append(nn.ReLU(inplace=True))
        stacked_convs.append(
    ...
    def forward_objdet_single(
        ...
        cls_score = conv_cls(
            cls_feat
        )  # (b, num_classes, h, w) shape. probabilities of class of the bbox at each grid.
        bbox_pred = conv_reg(
            reg_feat
        )  # (b, 6, h, w) shape. each grid contains one predicted bbox.
        objectness = conv_obj(
            reg_feat
        )  # (b, 1, h, w) shape, confidence of whether the object in the bbox is an object.
    ...
    def forward_keypts_single(
        ...
        aligned_rois = roi_align(
            left_feat, bbox_pred_list, output_size=self._config["aligned_roi_size"]
        )  # Note: output shape is (b*100, N, 14, 14)
        keypts_feat = keypts_convs(aligned_rois)  # Note: (b*100, num_of_class, 28, 28)
        keypts_prob_x = (
            torch.argmax(F.softmax(keypts_feat.sum(dim=-2), dim=-1), dim=-1).unsqueeze(
                -1
            )
            / keypts_feat.shape[-1]
        )  # Note: shape is (b*100, num_classes, 1)
        keypts_prob_y = (
            torch.argmax(F.softmax(keypts_feat.sum(dim=-1), dim=-1), dim=-1).unsqueeze(
                -1
            )
            / keypts_feat.shape[-2]
        )
        ...
        )  # Note: shape is (b*100, num_classes, 2)
