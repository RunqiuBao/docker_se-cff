...
class ResNetFeature(nn.Module):
    def __init__(
        ...
        stride = (
            3  # !!!Note: originally 3. bao changed for event based object training.
        )
        ...
        self.layer2 = self._make_layer(
            Bottleneck, base_channels * 2, layers[1], stride=2
        )  # H/6
        self.layer3 = self._make_layer(
            DeformBottleneck, base_channels * 4, layers[2], stride=2
        )  # H/12
...
class FeatureExtractor2(nn.Module):
    ...
    def forward(self, x):
        ...
        refined_feature = self._neck(feature)
