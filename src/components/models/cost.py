...
class CostVolume(nn.Module):
    ...
    def forward(self, left_feature, right_feature):
        ...
        if self.feature_similarity == "difference":
            ...
            for i in range(self.max_disp):
                if i > 0:
                    cost_volume[:, :, i, :, i:] = torch.cat(
                        (left_feature[:, :, :, i:], right_feature[:, :, :, :-i]), dim=1
                    )
                else:
            ...
            for i in range(self.max_disp):
                if i > 0:
                    ...
                    cost_volume[:, i + 1, :, :] = (left_feature * right_feature).mean(dim=1)
...
class CostVolumePyramid(nn.Module):
    ...
    def forward(self, left_feature_pyramid, right_feature_pyramid):
        ...
        for s in range(num_scales):
            ...
            cost_volume = cost_volume_module(
                left_feature_pyramid[s], right_feature_pyramid[s]
            )
