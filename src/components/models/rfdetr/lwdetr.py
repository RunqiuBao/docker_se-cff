...
class RFDETR(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__()
        backbone_cfg = network_cfg["backbone"]["params"]
        backbone = build_backbone(**backbone_cfg)
        transformer_cfg = network_cfg["transformer"]["params"]
        transformer = Transformer(**transformer_cfg)
        if network_cfg["segmentation_head"]["enabled"]:
            segmentation_head = SegmentationHead(**network_cfg["segmentation_head"]["params"])
        else:
            segmentation_head = None

        num_classes = network_cfg["num_classes"]
        num_queries = network_cfg["num_queries"]
        aux_loss = network_cfg["aux_loss"]
        group_detr = network_cfg["group_detr"]
        two_stage = network_cfg["two_stage"]
        lite_refpoint_refine = network_cfg["lite_refpoint_refine"]
        bbox_reparam = network_cfg["bbox_reparam"]
        ...
        # build loss instance
        self.grad_accum_steps: int = loss_cfg.PARAMS["grad_accum_steps"]
        self.criterion = build_criterion(
            self.device,
            network_cfg["num_classes"],
            **loss_cfg.PARAMS
        )
    ...
    def forward(self, x, labels=None, **kwargs):
        ...
        if labels is not None:
            losses, selected_detections = self.compute_loss(x, labels, **kwargs)
        artifacts = None
        if labels is not None:
            artifacts = [selected_detections]
        if self.is_freeze:
            losses = None
        return x, losses, artifacts

    def compute_loss(self, preds: torch.Tensor, labels: list[dict], **kwargs):
        for indexB, oneImageLabels in enumerate(labels):
            labels[indexB]["labels"] = oneImageLabels["labels"].to(dtype=torch.long)
        loss_dict, selected_detections = self.criterion(preds, labels)
        weight_dict = self.criterion.weight_dict
        losses = sum(
            (1 / self.grad_accum_steps) * loss_dict[k] * weight_dict[k]
            for k in loss_dict.keys()
            if k in weight_dict
        )
        return {"loss_weighted": losses}, selected_detections
...
class SetCriterion(nn.Module):
    ...
    def selected_boxes(self):
        return self._artifacts["selected_boxes"]

    @property
    def classes_of_selected(self):
        return self._artifacts["classes_of_selected"]

    @property
    def predicted_masks(self):
        return self._artifacts["predicted_masks"]

    @property
    def target_masks(self):
        return self._artifacts["target_masks"]
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.ia_bce_loss:
            ...
            self._artifacts["classes_of_selected"] = torch.argmax(prob[idx], dim=1)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
        self._artifacts["selected_boxes"] = src_boxes
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        self._artifacts["predicted_masks"] = src_masks
        self._artifacts["target_masks"] = target_masks

        num_points = max(src_masks.shape[-2], src_masks.shape[-2] * src_masks.shape[-1] // self.mask_point_sample_ratio)

        with torch.no_grad():
            # sample point_coords. Normalized coordinates in [0, 1]
    ...
    def forward(self, outputs, targets):
        ...
        if self.do_segmentation:
            return losses, {"bboxes": self.selected_boxes, "classes": self.classes_of_selected, "predicted_masks": self.predicted_masks, "target_masks": self.target_masks}
        else:
            return losses, {"bboxes": self.selected_boxes, "classes": self.classes_of_selected}
...
def build_criterion(
    ...
    mask_ce_loss_coef: float | None = None,
    mask_dice_loss_coef: float | None = None,
    ...
    **kwargs,
    ...
    if segmentation_head:
        criterion = SetCriterion(num_classes + 1, matcher=matcher, weight_dict=weight_dict,
                                ...
                                group_detr=group_detr, sum_group_losses=sum_group_losses,
