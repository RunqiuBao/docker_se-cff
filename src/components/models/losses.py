...
def varifocal_loss(pred: Tensor,
    ...
    this implementation considers bkg logits in pred.
...
@weighted_loss
def quality_focal_loss_withbkglogits(pred, target, beta=2.0):
    ...
    neg = (label == bg_class_ind).nonzero().squeeze(1)

    # negatives are supervised by 0 quality score
    pred_sigmoid = pred.sigmoid()
    negative_label = pred_sigmoid.new_zeros(pred.shape)
    negative_label[neg, -1] = 1.  # background category set to one
    scale_factor = pred_sigmoid - negative_label
    negative_label *= 10  # prob -> logits. (sigmoid(10)->1.0)
    loss = F.binary_cross_entropy_with_logits(
        pred, negative_label, reduction='none') * scale_factor.abs().pow(beta)  # downweighting easy ones (->0 are the easy ones)

    pos_label = label[pos].long()
    # positives are supervised by bbox quality (IoU) score
    scale_factor = score[pos] - pred_sigmoid[pos, pos_label]  # downweighting easy ones (->0 are the easy ones)
