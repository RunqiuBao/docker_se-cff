...
class ExpLogger:
    ...
    def write(
        self, log, file_name=None, mode="a", end="\n", is_print=True, add_time=True
    ):
    ...
    def save_args(self, args):
        ...
        self.write(
            log=args_log, file_name=self._FILE_NAME["args"], mode="w", add_time=False
        )

    def save_cfg(self, cfg):
        self.write(
            log=str(cfg), file_name=self._FILE_NAME["config"], mode="w", add_time=False
        )
    ...
    def log_model(self, model):
        ...
        self.write(
            log=str(model), file_name=self._FILE_NAME["model"], mode="w", add_time=False
        )
        self.write(
            log="Total number of parameters: %d" % num_params,
    ...
    def log_optimizer(self, optimizer):
        self.write(
            log=str(optimizer),
            file_name=self._FILE_NAME["optimizer"],
            mode="w",
            add_time=False,
        )
    ...
    def save_visualize(self, image, visual_type, sequence_name, image_name):
        visualize_root = os.path.join(
            self._save_root, self._DIR_NAME["visualize"], visual_type, sequence_name
        )
